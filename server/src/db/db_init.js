...
function createFileUploadFolder() {
  const folderPath = './src/public/fileUpload';

  // Check if the folder exists, and if not, create it
  if (!fs.existsSync(folderPath)) {
    fs.mkdirSync(folderPath);
    console.log('Created ./src/public/fileUpload folder.');
  } else {
    console.log('./src/public/fileUpload folder already exists.');
  }
}
  ...
  try {
    ...
    const createRecommendationsTableQuery = `
            CREATE TABLE IF NOT EXISTS recommendations (
                ...
                view_image VARCHAR(255) NOT NULL,
    ...
    // Create the ./src/public/fileUpload folder
