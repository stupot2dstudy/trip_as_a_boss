...
// Define a function to retrieve a user by their first name
export async function getUserByFirstName(firstName) {
    try {
        const connection = await pool.getConnection(); // Get a connection from the pool

        // Retrieve the user by their first name using a prepared statement
        const [rows] = await connection.query('SELECT * FROM users WHERE first_name = ?', [firstName]);
